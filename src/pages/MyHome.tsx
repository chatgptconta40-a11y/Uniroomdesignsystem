...
import {
  getHouseGroupConversation,
  getMessagesForConversation,
} from "../data/mockMessages";
import {
  getActiveHomeForStudent,
...
export function MyHome() {
  ...
  const { getProperty, getRoom, refreshProperties } =
    useProperties();
  ...
  if (!activeHome) {
    const acceptedApp = user
      ? getApplicationsForUser(user.id).find(
          (application) => application.status === "accepted",
        )
      : null;

    const acceptedRoom = acceptedApp?.roomId
      ? getRoom(acceptedApp.roomId)
      : undefined;
    const acceptedProperty = acceptedApp?.propertyId
      ? getProperty(acceptedApp.propertyId)
      : undefined;
          ...
          {acceptedApp ? (
            <Card className="overflow-hidden">
              {acceptedRoom && acceptedProperty && (
                <div className="relative h-48 bg-muted">
                  <img
                    src={
                      acceptedRoom.images[0] ||
                      acceptedProperty.images[0]
                    }
            ...
            <Card className="p-12 text-center">
              ...
              <h2 className="text-2xl font-bold mb-3">
                Ainda não tens uma casa ativa
              </h2>
              <p className="text-muted-foreground mb-8 max-w-md mx-auto">
                Quando uma candidatura for aceite e confirmares
                a estadia, esta página passa a mostrar todas as
                informações do teu alojamento.
              </p>
  ...
  const { property, room, accommodation, activeHomeData } =
    activeHome;
  const monthlyRent = activeHomeData.monthlyRent ?? room.price;
  const utilitiesAmount =
    activeHomeData.utilities ?? room.utilities ?? 0;
  const monthlyTotal = monthlyRent + utilitiesAmount;

  const paymentMethod = getPaymentMethodForHome(activeHomeData);
  const contract = getOrCreateRentalContract(
    activeHomeData,
    monthlyRent,
    utilitiesAmount,
  );
  const rentPayments = getRentPaymentsForHome(
    activeHomeData,
    monthlyRent,
    utilitiesAmount,
  );

  const housemates = readHousemates(
    property.id,
    room.roomNumber,
  );
  const nextPayment =
    rentPayments.find((payment) => payment.status !== "paid") ||
    rentPayments[0];
  const paidPayments = rentPayments.filter(
    (payment) => payment.status === "paid",
  );
  const paymentsWithProof = rentPayments.filter((payment) =>
    Boolean(payment.proofUrl),
  );

  const visibleMaintenanceRequests = maintenanceRequests.filter(
    (request) =>
  ...
  const pendingRequests = visibleMaintenanceRequests.filter(
    (request) => request.status === "pending",
  ).length;
  const inProgressRequests = visibleMaintenanceRequests.filter(
  ...
  const handleUploadPaymentProof = (paymentId: string) => {
    const updated = uploadPaymentProof(
      paymentId,
      `comprovativo-${new Date().toISOString().slice(0, 10)}.pdf`,
    );
  ...
  const amenities = [
    ...
    {
      label: "Aquecimento",
      value: property.amenities.heating
        ? "Disponível"
        : "Não disponível",
      icon: Flame,
    },
    {
      label: "Casa de banho",
      value: room.privateBathroom ? "Privativa" : "Partilhada",
      icon: Bath,
    },
    {
      label: "Moradores",
      value: `${housemates.length + 1} pessoas`,
      icon: Users,
    },
  ];
  ...
  const rules = [
    property.houseRules?.smoking
      ? "Fumar permitido apenas nas zonas indicadas"
      : "Não é permitido fumar dentro de casa",
    property.houseRules?.pets
      ? "Animais permitidos mediante aviso prévio"
      : "Animais não permitidos",
    property.houseRules?.parties
      ? "Eventos permitidos com acordo dos moradores"
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Gere a tua estadia, pagamentos, colegas de casa e
            ...
            <div className="relative min-h-[280px]">
              ...
              <div className="absolute left-6 right-6 bottom-6 text-white">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <Badge
                    variant="success"
                    className="bg-white/95 text-secondary border-white/60"
                  >
            ...
            <div className="p-6 lg:p-8">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="p-4 bg-muted/40 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {formatCurrency(utilitiesAmount)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="font-semibold text-foreground">
                    {formatDate(activeHomeData.moveInDate)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span className="text-muted-foreground flex items-center gap-2">
                  ...
                  <span className="font-semibold text-foreground">
                    {formatDate(activeHomeData.contractEndDate)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span className="text-muted-foreground flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 space-y-6">
            <Card className="p-6">
              <div className="flex items-center justify-between gap-4 mb-5">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Informação prática da casa partilhada.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  ...
                  >
                    <CheckCircle
                      className={`w-4 h-4 ${enabled ? "text-secondary" : "text-muted-foreground"}`}
                    />
                    <span
            ...
            {(() => {
              const houseChat = getHouseGroupConversation(
                property.id,
                user?.id || "",
              );
              if (!houseChat) return null;

              const lastMessages = getMessagesForConversation(
                houseChat.id,
              ).slice(-3);
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    {houseChat.unreadCount > 0 && (
                      ...
                      >
                        {houseChat.unreadCount} nova
                        {houseChat.unreadCount > 1 ? "s" : ""}
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 mb-4">
                    {lastMessages.length === 0 ? (
                      <div className="text-center py-6 text-muted-foreground text-sm">
                        Nenhuma mensagem ainda. Começa a
                        conversa!
                      </div>
                    ) : (
                  ...
                  <Link
                    to={`/messages?conversation=${houseChat.id}`}
                    className="block"
                  >
                    <Button
                      variant="primary"
            ...
            <Card className="p-6">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Acompanha problemas reportados no quarto ou
                    na casa.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {inProgressRequests > 0 && (
                    <Badge variant="default">
                      {inProgressRequests} em resolução
                    </Badge>
                  )}
                  {urgentRequests > 0 && (
                    <Badge
              ...
              {visibleMaintenanceRequests.length > 0 ? (
                <div className="space-y-3">
                  {visibleMaintenanceRequests.map((request) => (
                    <div
                      key={request.id}
                      className="p-5 border border-border rounded-lg hover:border-primary/40 transition-all"
                    >
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-6 max-w-md mx-auto">
                    Quando tiveres algum problema no alojamento,
                    podes reportá-lo aqui.
                  </p>
                  <Button
                    variant="primary"
                    onClick={() =>
          ...
          <div className="space-y-6">
            ...
            <Card
              key={`payment-${financeRefreshKey}`}
              className="p-0 overflow-hidden border-primary/10"
            >
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  nextPayment.status !== "paid" ? (
                    <Button
                      size="sm"
                      onClick={() =>
                        handleUploadPaymentProof(nextPayment.id)
                      }
                    >
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    A UniRoom não processa pagamentos. Usa estes
                    dados e guarda sempre comprovativo.
                  </p>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-5">
              ...
              <div className="grid grid-cols-1 gap-2">
                <Button
                  ...
                  onClick={() =>
                    alert("A abrir contrato demonstrativo.")
                  }
                >
                  <FileText className="w-4 h-4 mr-2" />
                  Ver contrato
                ...
                <Button
                  variant="outline"
                  className="w-full justify-start"
                  disabled={paymentsWithProof.length === 0}
                >
                  <Receipt className="w-4 h-4 mr-2" />
                  {paymentsWithProof.length} comprovativo
                  {paymentsWithProof.length === 1 ? "" : "s"}{" "}
                  enviado
                  {paymentsWithProof.length === 1 ? "" : "s"}
                </Button>
              </div>
