...
export function AdminDashboard() {
  ...
  const verificationRate = allLandlords.length > 0
    ? Math.round((verifiedLandlords.length / allLandlords.length) * 100)
    : 0;
  const moderationLoad = openReports + pendingApplications + suspendedProperties.length;
  ...
  const operationalScorePercent = Math.round((operationalScore / 5) * 100);

  const priorityQueue = [
    criticalReports > 0 && {
      label: `${criticalReports} denúncia${criticalReports > 1 ? 's' : ''} crítica${criticalReports > 1 ? 's' : ''}`,
      detail: 'Requer validação manual antes de qualquer outra tarefa.',
      route: '/admin/reports',
    ...
    pendingApplications > 0 && {
      ...
      detail: 'Sinal de carga operacional entre estudantes e senhorios.',
    ...
    suspendedProperties.length > 0 && {
      ...
      detail: 'Rever impacto nos senhorios e disponibilidade da plataforma.',
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-4">
        <Card className="p-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
            <div>
              <p className="text-sm font-medium text-blue-600 mb-1">Centro de controlo</p>
              <h2 className="text-lg font-bold text-gray-900">Fila de trabalho administrativa</h2>
              <p className="text-sm text-gray-500">
                Tarefas que a equipa deve resolver primeiro para manter a plataforma segura.
              </p>
            </div>
          ...
          {priorityQueue.length > 0 ? (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs opacity-80 mt-1 leading-relaxed">{item.detail}</p>
                      </div>
                    </div>
                  </button>
              ))}
            ...
            <div className="rounded-xl border border-green-200 bg-green-50 p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-green-800">Sem prioridades urgentes</p>
                <p className="text-xs text-green-700 mt-1">
                  Não há denúncias críticas, bloqueios relevantes ou filas operacionais por tratar.
                </p>
              </div>
            </div>
          )}
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              <h2 className="text-base font-bold text-gray-900">Saúde operacional</h2>
              <p className="text-xs text-gray-500">Resumo de risco e carga da plataforma.</p>
            </div>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              criticalReports > 0 ? 'bg-red-100 text-red-600' : moderationLoad > 0 ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'
            }`}>
              {criticalReports > 0 ? <ShieldAlert className="w-5 h-5" /> : moderationLoad > 0 ? <AlertCircle className="w-5 h-5" /> : <ShieldCheck className="w-5 h-5" />}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="rounded-lg bg-gray-50 p-3">
              ...
              <p className="text-sm font-bold text-gray-900">{operationalScorePercent}%</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-3">
              <p className="text-[11px] text-gray-500">Carga</p>
              <p className="text-sm font-bold text-gray-900">{moderationLoad}</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-3">
              <p className="text-[11px] text-gray-500">Verificação</p>
              <p className="text-sm font-bold text-gray-900">{verificationRate}%</p>
            </div>
          </div>

          <div className="space-y-3">
            {[
              { label: 'Denúncias críticas', ok: criticalReports === 0, value: criticalReports === 0 ? 'OK' : `${criticalReports} abertas` },
              { label: 'Fila de denúncias', ok: openReports <= 3, value: `${openReports} abertas` },
              { label: 'Candidaturas pendentes', ok: pendingApplications <= 5, value: `${pendingApplications}` },
              { label: 'Senhorios verificados', ok: verificationRate >= 50, value: `${verificationRate}%` },
              { label: 'Taxa de ocupação', ok: occupancyRate >= 50, value: `${occupancyRate}%` },
            ].map(check => (
              <div key={check.label} className="flex items-center gap-3">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  check.ok ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'
                }`}>
                  {check.ok ? <CheckCircle className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                </div>
                <span className="text-sm text-gray-700 flex-1 truncate">{check.label}</span>
                <span className={`text-xs font-semibold ${check.ok ? 'text-green-700' : 'text-amber-700'}`}>
                  {check.value}
                </span>
              </div>
            ))}
